...
def sample_Msc_power_law(N, alpha, Msc_min, Msc_max, grid_size):
    def p(Msc):
        return 10**(5.32) * Msc**(alpha)        
    
    Msc_vals = np.linspace(Msc_min, Msc_max, grid_size)    
    p_vals = p(Msc_vals)
    p_vals /= np.trapz(p_vals, Msc_vals)
    cdf_vals = np.cumsum(p_vals)
    cdf_vals /= cdf_vals[-1]
    return np.interp(np.random.rand(N), cdf_vals, Msc_vals)
    
# Mass ratio
def sample_mass_ratio(N, q_min=0.5, q_max=1.0):
...
def generate_synthetic_MBHB_population(N, geoctime_i, geoctime_f,                                       
                                       alpha=-1.30, #-1.20 yielded 320 detected mergers in 4 years; original -0.90
    ...
    Msc = sample_Msc_power_law(N, alpha, Msc_min, Msc_max, grid_size)
