...
def sample_mass1_power_law(N, alpha, m_min, m_max, grid_size):
    def p(m):
        return 10**(5.32) * m**(alpha)        
...
def generate_synthetic_MBHB_population(N, geoctime_i, geoctime_f,                                       
                                       alpha=-1.20, #-1.20 yielded 320 detected mergers in 4 years; original -0.90
                                       m_min=1e3, m_max=1e8,
                                       q_min=0.9, q_max=1.0,
    ...
    m1 = sample_mass1_power_law(N, alpha, m_min, m_max, grid_size)
