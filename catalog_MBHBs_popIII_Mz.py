...
def sample_broken_power_law(N, alpha1, alpha2, Mz_min, Mz_break, Mz_max, grid_size):
    def p(Mz):
        return np.where(
            Mz <= Mz_break,
            10**(-10.6) * Mz**(alpha1),
    ...
    Mz_vals = np.linspace(Mz_min, Mz_max, grid_size)    
    p_vals = p(Mz_vals)
    p_vals /= np.trapz(p_vals, Mz_vals)
    cdf_vals = np.cumsum(p_vals)
    cdf_vals /= cdf_vals[-1]
    return np.interp(np.random.rand(N), cdf_vals, Mz_vals)
...
def generate_synthetic_MBHB_population(N, geoctime_i, geoctime_f,                                       
    ...
    Mz = sample_broken_power_law(N, alpha1, alpha2, Mz_min, Mz_break, Mz_max, grid_size)
